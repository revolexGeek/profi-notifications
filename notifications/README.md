...
```jsonc
{
  ...
  "messageThreadId": 2                // опционально: ID темы форума (переопределяет дефолт)
}
```

Неизвестные поля игнорируются. Невалидный payload (нет `text`, неверный тип,
пустой текст, длиннее 4096, `messageThreadId` не положительное целое)
отправляется в DLQ без ретраев.

### Группы с темами (форумы) и каналы

`TELEGRAM_CHAT_ID` — это либо `@username`/`-100…` канала, либо `-100…`
супергруппы с темами. Разбор приватной ссылки, например `https://t.me/c/1234567890/2`:

- `chat_id` = `-100` + число после `/c/` → `-1001234567890`;
- `2` — это `message_thread_id` (ID темы), а **не** номер сообщения.

Тему по умолчанию задаёт `TELEGRAM_MESSAGE_THREAD_ID`; отдельное сообщение может
переопределить её полем `messageThreadId`. Если тема не задана нигде — сообщение
уходит в тему «General» (или в обычный чат/канал). Бот должен состоять в группе и
иметь право писать в целевую тему.
...
| `TELEGRAM_CHAT_ID` | — | Канал (`@username`/`-100…`) или супергруппа с темами (`-100…`). |
| `TELEGRAM_MESSAGE_THREAD_ID` | — | Тема форума по умолчанию (`message_thread_id`). Опционально; переопределяется полем `messageThreadId` в payload. |
